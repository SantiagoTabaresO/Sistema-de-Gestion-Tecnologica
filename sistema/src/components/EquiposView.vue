<template>
  <div class="dashboard-container">
    ...
    <div class="main-content">
      ...
      <div class="header">
        <h1>
          <span><i class="fas fa-laptop-medical"></i></span> Equipos Médicos
        </h1>
      </div>
      ...
      <div class="search-bar">
        <input 
          type="text" 
          placeholder="Buscar por número de activo, marca o modelo"
          v-model="busqueda"
        />
      </div>
      ...
      <div class="table-container">
        <h2>Listado de Equipos ({{ equiposFiltrados.length }} registros)</h2>
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="equipo in equiposFiltrados" :key="equipo.id">
              ...
              <td>{{ equipo.marca }}</td>
              <td>{{ equipo.modelo }}</td>
              <td>{{ getNombreUbicacion(equipo.codigo_ubicacion) }}</td>
              <td>{{ getNombreResponsable(equipo.codigo_responsable) }}</td>
              <td>
                <router-link 
                  v-if="equipo.id"
                  :to="{ name: 'EditarEquipoView', params: { id: equipo.id } }" 
        ...
        <div v-if="equiposFiltrados.length === 0" class="text-center p-4 text-muted">
      ...
      <div class="action-section">
        ...
        <div class="delete-area">
          <label for="codigo-eliminar">
            Escriba el <b>número de activo</b> del equipo a 
...
export default {
  ...
  computed: {
    equiposFiltrados() {
      if (!this.busqueda) return this.equipos;
      return this.equipos.filter(e =>
        e.numero_activo?.toLowerCase().includes(this.busqueda.toLowerCase()) ||
  ...
  methods: {
    async cargarDatos() {
      try {
        ...
        this.equipos = Array.isArray(equiposData) ? equiposData : [];
        this.ubicaciones = Array.isArray(ubicData) ? ubicData : [];
        this.responsables = Array.isArray(respData) ? respData : [];
      } catch (error) {
        console.error("Error cargando datos:", error);
      }
    },

    getNombreUbicacion(codigo) {
      const ubic = this.ubicaciones.find(u => u.codigo_asignado === codigo);
      return ubic ? ubic.nombre_ubicacion : codigo;
    },

    getNombreResponsable(codigo) {
      const resp = this.responsables.find(r => r.codigo_asignado === codigo);
      return resp ? resp.nombre_apellidos : codigo;
    },

    async borrarEquipoPorCodigo() {
      const codigo = this.codigoAEliminar.trim();
      const equipo = this.equipos.find(e => e.numero_activo === codigo);

      if (!equipo) {
        alert(`No se encontró un equipo con el número de activo: ${codigo}.`);
        return;
      }
